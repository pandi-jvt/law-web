...
export default function Profile() {
  ...
  const roleLabel = formatLabel(user?.groupRole || user?.group_role || user?.role);
  const accountLabel = formatLabel(user?.accountType || user?.account_type);
  const profileLabel = [roleLabel, accountLabel].filter(Boolean).join(' • ') || user?.email || 'Signed in';
  ...
  return (
    <Box ref={rootRef} sx={{ flexShrink: 0, ml: 'auto' }}>
